%LDG scheme with central flux for second order
clear
clc
n=[10 20 40]; %number of elements
testnum=length(n);
xleft=0;
xright=2*pi;
tleft=0;
    ...
    for j=1:n(i)
        ...
        for m=0:k
            ...
            for l=0:k
                ...
                A(m+1,l+1)=integral(first,xjmh,xjph)-basislj(xjph).*basismj(xjph)/2+basislj(xjmh).*basismj(xjmh)/2;
                B(m+1,l+1)=-integral(second,xjmh,xjph);
                C(m+1,l+1)=basisljm(xjmh)*basismj(xjmh)/2;
                DD(m+1,l+1)=-basisljp(xjph)*basismj(xjph)/2;
            end
        end
        A2=B\A;
        C2=B\C;
        ...
        A3=C2^2;
        B3=A2*C2+C2*A2;
        C3=A2^2+C2*DD2+DD2*C2;
        D3=A2*DD2+DD2*A2;
        E3=DD2^2;
        D((j-1)*(k+1)+1:j*(k+1),(j-1)*(k+1)+1:j*(k+1))=C3;
        if (j>2&&j<n(i)-1)
            D((j-1)*(k+1)+1:j*(k+1),(j-3)*(k+1)+1:(j-2)*(k+1))=A3;
            D((j-1)*(k+1)+1:j*(k+1),(j-2)*(k+1)+1:(j-1)*(k+1))=B3;
            D((j-1)*(k+1)+1:j*(k+1),j*(k+1)+1:(j+1)*(k+1))=D3;
            D((j-1)*(k+1)+1:j*(k+1),(j+1)*(k+1)+1:(j+2)*(k+1))=E3;
        elseif (j==1)
            D((j-1)*(k+1)+1:j*(k+1),(n(i)-2)*(k+1)+1:(n(i)-1)*(k+1))=A3;
        ...
        elseif (j==n(i))
            ...
            D((j-1)*(k+1)+1:j*(k+1),(k+1)+1:2*(k+1))=E3;
        elseif (j==2)
            D((j-1)*(k+1)+1:j*(k+1),(n(i)-1)*(k+1)+1:n(i)*(k+1))=A3;
            D((j-1)*(k+1)+1:j*(k+1),(j-2)*(k+1)+1:(j-1)*(k+1))=B3;
            D((j-1)*(k+1)+1:j*(k+1),j*(k+1)+1:(j+1)*(k+1))=D3;
            D((j-1)*(k+1)+1:j*(k+1),(j+1)*(k+1)+1:(j+2)*(k+1))=E3;
        elseif (j==n(i)-1)
...
plot(x',u)
legend('uh','u')
title('P2 Solution for N=40 at t=1')
hold off