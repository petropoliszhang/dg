    ...
    for j=1:n(i)
        ...
        for m=0:k
            ...
            for l=0:k
                ...
                first=@(x) basislj(x).*basisgradmj2(x); 
                second=@(x) basislj(x).*basismj(x);
                A(m+1,l+1)=integral(first,xjmh,xjph)-basislj(xjph).*basismj(xjph)/dx*alpha-basisgradlj(xjmh).*basismj(xjmh)-basismj(xjmh).*basislj(xjmh)/dx*alpha-basisgradmj(xjph).*basislj(xjph);
                B(m+1,l+1)=integral(second,xjmh,xjph);
                C(m+1,l+1)=basisljm2(xjmh)*basisgradmj(xjmh)+basismj(xjmh)*basisljm2(xjmh)/dx*alpha;
                DD(m+1,l+1)=basisljp(xjph)*basismj(xjph)+basismj(xjph)*basisljp2(xjph)/dx*alpha;
            end
        end
        A2=B\A;
        C2=B\C;
